...
type DefaultStatus = 'Принято' | 'Готовится' | 'Доставляется' | 'Доставлено'
...
export  function OrderCardAdmin<T extends DefaultOrder = DefaultOrder>({
  order,
  onUpdate,
}: {
  order: T
  ...
  const STATUS_ORDER: DefaultStatus[] = ['Принято', 'Готовится', 'Доставляется', 'Доставлено']

  function itemsPreview(items: DefaultItem[]) {
    if (!Array.isArray(items) || items.length === 0) return ''

    return items.map((it) => `${it.name}${it.variant ? ` · ${it.variant}` : ''} - x${it.count}`).join(', ')
  }

  async function patchStatus(orderId: string, status: DefaultStatus) {
  ...
  const handleQuickToggle = async () => {
    const idx = STATUS_ORDER.indexOf(order.status as DefaultStatus)

    if (idx >= STATUS_ORDER.length - 1) return
    const next = STATUS_ORDER[idx + 1]

    onUpdate?.(order.id, { status: next as T['status'], updatedAt: new Date().toISOString() } as Partial<T>)
    setIsSaving(true)
    ...
    try {
      ...
      onUpdate?.(order.id, { status: order.status, updatedAt: order.updatedAt } as Partial<T>)
    } finally {
      setIsSaving(false)
    }
  }

  const handleSave = async (status: DefaultStatus) => {
    ...
    onUpdate?.(order.id, { status: status as T['status'], updatedAt: new Date().toISOString() } as Partial<T>)
    ...
    try {
      ...
      onUpdate?.(order.id, { status: prev, updatedAt: order.updatedAt } as Partial<T>)
    } finally {
      setIsSaving(false)
    }
  }
